-- Migration 0019: Admin Room Analytics
-- Provides RPC functions for aggregating room revenue and performance metrics.

-- 1. Get Room-Level Analytics
-- Aggregates revenue, booking counts, and nights sold per room for a given period.
-- Attribution is based on Check-In date falling within the period.
...
RETURNS TABLE (
  ...
  nights_sold bigint,
  occupancy_rate numeric -- Percentage of days in period occupied (approximate)
...
BEGIN
  ...
  WITH bookings AS (
    ...
    WHERE entity_type = 'front_desk'
      ...
      AND status IN ('approved', 'checked_out', 'completed', 'archived') -- Include all valid finalized statuses
