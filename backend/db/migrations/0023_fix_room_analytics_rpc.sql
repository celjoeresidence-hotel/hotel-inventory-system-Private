-- Migration 0023: Fix Room Analytics RPC
-- Fixes the invalid enum input error in get_room_analytics by restricting status check to 'approved'.
...
BEGIN
  ...
  WITH bookings AS (
    ...
    WHERE entity_type = 'front_desk'
      ...
      AND status = 'approved' -- Only count approved bookings (removes invalid enum check)
